...
namespace Kudu.Core.Tracing
{
    public class CascadeTracer : ITracer
    {
        ...
        public CascadeTracer(params ITracer[] tracers)
        {
            if (tracers.Length == 0)
            {
                // constructor should not be called with an empty array
                // but we don't want to throw exception when there's a tracer error
                this.tracers = new ITracer[] { NullTracer.Instance };
            }
            else
            {
