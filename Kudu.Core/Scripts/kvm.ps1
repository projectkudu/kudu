...
param(
  ...
  [string] $value
    ...
    [System.Console]::WriteLine("Setting alias '$name' to '$kreFullName'")
    md ($userKrePath + "\alias\") -Force | Out-Null
...
#      "list 0"            {Kvm-Global-List}
      ...
      default             {[System.Console]::WriteLine('Unknown command, or global switch not supported'); Kvm-Help;}
    }
   } else {
    switch -wildcard ($command + " " + $args.Count) {
      "setup 0"           {Kvm-Global-Setup}
      ...
      "help 0"              {Kvm-Help}
      " 0"              {Kvm-Help}
      default             {[System.Console]::WriteLine('Unknown command'); Kvm-Help;}
    }
   }
  }
  catch {
