...
namespace Kudu.Core.Jobs
{
    ...
    public abstract class JobsManagerBase<TJob> : JobsManagerBase, IJobsManager<TJob>, IDisposable, IRegisteredObject where TJob : JobBase, new()
    {
        ...
        private DateTime JobListCacheExpiryDate;

        private const int JobListCacheTimeOutInMinutes = 10;
        ...
        public IEnumerable<TJob> ListJobs(bool forceRefreshCache)
        {
            ...
            {
                if (jobList == null || forceRefreshCache || JobListCacheExpiryDate < DateTime.Now)
                {
                    jobList = ListJobsInternal();
                    JobListCache = jobList;
                    JobListCacheExpiryDate = DateTime.Now.AddMinutes(JobListCacheTimeOutInMinutes);
                }
            }
            return jobList;
        }
