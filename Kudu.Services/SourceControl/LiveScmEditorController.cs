...
{
    /// <summary>
    /// A Virtual File System controller which exposes GET and PUT for part of the Kudu file system
    ...
    public class LiveScmEditorController : VfsControllerBase
    {
        ...
        public override HttpResponseMessage GetItem()
        {
            ...
            {
                return Request.CreateErrorResponse(HttpStatusCode.Conflict, "The repository is currently busy handling another request. Please try again later.");
            }
            try
            {
        ...
        public override Task<HttpResponseMessage> PutItem()
        {
            ...
            if (!_operationLock.Lock())
            {
                HttpResponseMessage busyResponse = Request.CreateErrorResponse(HttpStatusCode.Conflict, "The repository is currently busy handling another request. Please try again later.");
        ...
        {
            // If repository is empty then there is no commit id and no master branch so we don't create any branch
            if (_currentEtag != null)
            {
                ...
                if (itemExists)
                {
                    ...
                    {
                        HttpResponseMessage noIfMatchResponse = Request.CreateErrorResponse(
                            HttpStatusCode.PreconditionFailed,
                            "Updating an existing resource requires an If-Match header carrying a single etag based on an existing commit ID.");
                        noIfMatchResponse.Headers.ETag = _currentEtag;
                        return TaskHelpers.FromResult(noIfMatchResponse);
                    }

                    EntityTagHeaderValue ifMatch = Request.Headers.IfMatch.First();
                    if (ifMatch.IsWeak || ifMatch.Tag == null)
                    {
                        HttpResponseMessage weakEtagResponse = Request.CreateErrorResponse(
                            HttpStatusCode.PreconditionFailed,
                            "Updating an existing resource requires a strong etag based on an existing commit ID.");
                        weakEtagResponse.Headers.ETag = _currentEtag;
                        return TaskHelpers.FromResult(weakEtagResponse);
                    }
                ...
                catch
                {
                    HttpResponseMessage conflictFileResponse = Request.CreateErrorResponse(
                        HttpStatusCode.PreconditionFailed,
                        "Commit ID not found. Updates must be based on an existing commit ID");
                    conflictFileResponse.Headers.ETag = _currentEtag;
                ...
                return Request.Content.CopyToAsync(fileStream)
                    .Then(() =>
                    {
                        ...
                        catch (Exception e)
                        {
                            ...
                            HttpResponseMessage conflictResponse = Request.CreateErrorResponse(HttpStatusCode.Conflict, e);
                            _readStream = new RepositoryItemStream(this, GetFileReadStream(localFilePath));
        ...
        private EntityTagHeaderValue GetCurrentEtag()
        {
            ...
            try
            {
                return CreateEtag(_repository.CurrentId);
            }
            catch
            {
                return null;
