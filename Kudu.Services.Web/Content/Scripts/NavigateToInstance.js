        ...
        success: function (response) {
            try {
                ...
                for (var i = 0; i < obj.length; i++) {
                    ...
                    instanceTabBtn.innerHTML = '<a href=\"#" onclick="NavigateToInstance(\'' + obj[i] + '\')"> Instance Id:' + obj[i].substring(0, 4) + '</a>';
                    instanceTabBtn.setAttribute("id", "inst-id-btn-" + obj[i]);
                    if (obj[i].trim().valueOf() == $.currInst) {
                        $("#instance-drop-down-text").text("Instance Id: " + obj[i].substring(0, 4));
                    }
